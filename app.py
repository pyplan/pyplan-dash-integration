...
from utils import createRangeSliderMarks, createDashSelectorOptions, default_colors, default_layout, read_source_code
...
MODEL_PATH = PATH.joinpath("pyplan-model").resolve()

pyplan_model_file = os.path.join(
    MODEL_PATH, "getting started with planning v11.ppl")
pyplan = Pyplan()
pyplan.openModel(pyplan_model_file)
...
@app.callback(Output('output_upload', 'children'),
              [Input('upload_model', 'contents')],
              [State('upload_model', 'filename'),
               State('upload_model', 'last_modified')])
def upload_pyplan_model(content, filename, dates):
    res = ""
    if content:
        res = "The file must be a valid pyplan model file"
        if str(filename).endswith(".ppl"):
            content_arr = content.split(',')
            if len(content_arr) > 0:
                content_string = content_arr[1]
                decoded = b64decode(content_string)
                new_model = os.path.join(MODEL_PATH, "new_temp_model.ppl")
                with open(new_model, 'w') as f:
                    f.write(decoded.decode('utf-8'))
                pyplan.openModel(new_model)
                res = "The new model has been loaded"
    return res
